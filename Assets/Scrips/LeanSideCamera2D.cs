	...
	public class LeanSideCamera2D : MonoBehaviour
	{
		...
		protected virtual void LateUpdate()
		{
			...
			if (Camera.main != null)
			{
				...
				Camera.main.transform.position -= new Vector3(0,worldDelta.y,0);

				if(Camera.main.transform.position.y>=0){
					Camera.main.transform.position = new Vector3(Camera.main.transform.position.x,0,Camera.main.transform.position.z);
				}
				if(Camera.main.transform.position.y<=limINF){
					Camera.main.transform.position = new Vector3(Camera.main.transform.position.x,limINF,Camera.main.transform.position.z);
				}
